...
public class InterimPlanAdapter extends RecyclerView.Adapter<InterimPlanAdapter.ViewHolder>{
    ...
    public InterimPlanAdapter(List<DevicePlanListBean> lists, Context mContext){
        this.list = lists;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if ("待下达".equals(list.get(position).getPlanStatus())){
            ...
            holder.name2.setVisibility(View.VISIBLE);
            holder.name2.setText("拒绝原因:");

            if (list.get(position).getRejectReason() == null){

                holder.value2.setVisibility(View.GONE);
            }else {
                holder.value2.setVisibility(View.VISIBLE);
                holder.value2.setText(list.get(position).getRejectReason().toString());
            }
