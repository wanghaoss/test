...
public class NewTpBuyCheckDetailsAdapter extends BaseExpandableListAdapter {

    private List<BuyCheckDetailsModel.DetailListBean> mData;
    ...
    public NewTpBuyCheckDetailsAdapter(Context mcontext, String type,View rootView) {
    ...
    public interface OnClickBtItem {
        ...
        void myOrderTpPassClick(int position, BuyCheckDetailsModel.DetailListBean.BillDetailListBean singleData);
    }
     ...
     * 添加数据
     **/
    public void updata(List<BuyCheckDetailsModel.DetailListBean> data) {
        mData = data;
    }
    ...
    public int getChildrenCount(int groupPosition) {
        ...
        return mData.get(groupPosition) == null ? 0 : mData.get(groupPosition).getBillDetailList().size();
    ...
    public Object getChild(int groupPosition, int childPosition) {
        if (mData != null && mData.size() > 0) {
            List<BuyCheckDetailsModel.DetailListBean.BillDetailListBean> childs = mData.get(groupPosition).getBillDetailList();
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = View.inflate(mContext, R.layout.item_tp_buy_check_list_status, null);
            childViewHolder = new ChildViewHolder(convertView);
            convertView.setTag(childViewHolder);
        } else {
            childViewHolder = (ChildViewHolder) convertView.getTag();
        }
        childViewHolder.setData(childPosition, mData.get(groupPosition).getBillDetailList().get(childPosition));
        return convertView;
    }
    ...
    class GroupViewHolder {
        ...
        void setData(final int positions, final  BuyCheckDetailsModel.DetailListBean singleData) {
...
//            if (ExtraConfig.IntentType.FROM_INTENT_BY_BEIHUO.equals(fromType)){
            ...
            mTvPass.setTextColor(getColorByStatus(singleData.getStatus() ));
            ...
            mTvPass.setBackground(getBtBgByStatus(singleData.getStatus()));
...
//            }
            ...
            mTvPass.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    onClickBtItem.myOrderTpPassClick(positions,singleData);
                }
            });
    ...
    /**
     *
     * @param status  //采购验收单状态：0待验收 1通过2 ->不通过 3托盘不通过
     * @return 根据状态选择不同的颜色
     */
    private int getColorByStatus(String status){
        ...
        if ("0".equals(status)){
            ...
            return mContext.getResources().getColor(R.color.font_orange);
        }
    ...
    private Drawable getBtBgByStatus(String status){
        ...
        if ("0".equals(status)){
            ...
            return mContext.getResources().getDrawable(R.drawable.shape_rectangle_line_orange_bg_white);
        }
