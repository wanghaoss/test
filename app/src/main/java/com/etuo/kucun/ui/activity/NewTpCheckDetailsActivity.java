...
public class NewTpCheckDetailsActivity extends BaseActivity implements NewTpCheckDetailsAdapter.OnClickBtItem {
...
//
                        if (null != responseData) {
                            ...
                            orderNum = mOutBoundModel.getPalletCheckNo();
...
//                            lvOutList.onRefreshComplete();
//                        }
                        ShowToast.tCustom(NewTpCheckDetailsActivity.this, e.getMessage());
                    }
                });
...
//            lvOutList.getRefreshableView().setSelectedChild(ints[0],ints[1],false);
    ...
    public void upLoadOrderComfrim() {
        ...
        OkGo.post(UrlTools.getInterfaceUrl(CHECK_BILL_CHECK))
                ...
                .execute(new DialogCallback<LzyResponse<String>>(this, "加载中..") {
                    ...
                    public void onError(Call call, Response response, Exception e) {
                        super.onError(call, response, e);
                        ShowToast.tCustom(NewTpCheckDetailsActivity.this, NullToStr(e.getMessage()));
                    }
                });
    }
     ...
     */
    public void commitListBing(String palletList,final String palletNum,final String type) {
        LogUtil.d("token : " + PreferenceCache.getToken());
        OkGo.post(UrlTools.getInterfaceUrl(TP_PALLET_CHECK))
                ...
                .execute(new DialogCallback<LzyResponse<String>>(this, "加载中..") {
                    ...
                    public void onError(Call call, Response response, Exception e) {
                        ...
                        ShowToast.tCustom(NewTpCheckDetailsActivity.this, e.getMessage().toString());
                    }
                });
    }
    ...
    public void myOrderPassAllByModelClick(int position, final TpCheckDetailsModel.PalletModelListBean singleData) {
        ...
        if (!isOkByAllStatus(singleData.getCheckBillPalletList(),"2")){
            PromptHintCancel p = new PromptHintCancel(this) {
                @Override
                protected void onOk() {
                    getCanWriteOrClearList(singleData.getCheckBillPalletList(),"2");
...
//                    commitListBing("",singleData.getModelId(),"","","2","1",TYPE_BY_COMMIT_LIST);
    ...
    private String getCommitJosn(List<TpCheckDetailsModel.PalletModelListBean.CheckBillPalletListBean> singleData,String status){
        ...
        for (int i = 0;i<singleData.size();i++){
            ...
            commitModel.setPalletNum(singleData.get(i).getPalletNum());
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (keyCode == 280 && event.getRepeatCount() == 0) {//点击了 扳机
            if (MORE_TYPE.equals(checkTypeScanTp)) {
                ...
                LogUtil.d(" 扳机 松开 开始提交  " + System.currentTimeMillis());

                if (null != detailListBeanList && detailListBeanList.size() > 0) {

                    commitListBing(getCommitJosn(detailListBeanList,"2"),"",TYPE_BY_COMMIT_LIST);
                } else {
    ...
    private void initRfid() {
        ...
        mRfidOperationUtils.readTagCallBack(new RfidOperationUtils.readTagCallBack() {
            ...
            public void MoreStepReadTagCallBack(String strTid, String strEpc) {
                ...
                TpCheckDetailsModel.PalletModelListBean.CheckBillPalletListBean singleData = getCheckBean(strEpc, "2");
...
//                    commitListBing("","","1",getCommitJosn(newDetailListBeen,"3"),"",TYPE_BY_COMMIT_LIST);
                }
            } else {
