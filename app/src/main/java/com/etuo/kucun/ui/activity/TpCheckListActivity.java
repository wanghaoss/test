...
import com.etuo.kucun.ui.adapter.CheckTpListAdapter;
...
public class TpCheckListActivity extends BaseHeaderBarActivity {
    @BindView(R.id.lv_massagelist)
    PullToRefreshListView lvMassagelist;
    @BindView(R.id.im_no_date)
    LinearLayout imNoDate;
    private CheckTpListAdapter mAdapter;
    private List<TpCheckListModel> listModels = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ExtraConfig.IntentType.FROM_INTENT_BY_SHOU.equals(FromIntent)){
            setHeaderTitle("收托列表");

        }
        mAdapter = new CheckTpListAdapter(this,listModels,FromIntent);
    ...
    /**
     *  status 验收单状态 0待验收 2已验收 传空或不传时为全部（非必传）
     start 分页：起始页
     length 分页：每页条数
     */
    public void getData(final boolean first, final boolean isPullDown) {
        ...
        if (!isPullDown) {
            if (end) {
                setRefreshList(isPullDown, new ArrayList<TpCheckListModel>());
                return;
            } else {
        ...
        OkGo.get(UrlTools.getInterfaceUrl(UrlTools.TP_CHECK_LIST))
                .tag(this)
                ...
                .execute(new DialogCallback<LzyResponse<List<TpCheckListModel>>>(this, "加载中..") {
                    @Override
                    public void onSuccess(LzyResponse<List<TpCheckListModel>> responseData, Call call, Response response) {
                    ...
                    public void onError(Call call, Response response, Exception e) {
                        ...
                        ShowToast.tCustom(TpCheckListActivity.this, e.getMessage().toString());
                    }
                });
    }
     ...
     * @param result
     */
    private void setRefreshList(boolean isPullDown, List<TpCheckListModel> result) {
