...
public class MainActivity extends BaseActivity implements EasyPermissions.PermissionCallbacks {
    ...
    @OnClick({R.id.rl_shou, R.id.rl_huan,R.id.rl_ruku,
            R.id.rl_tpzk,R.id.rl_market_out,})
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.rl_shou:
                //收托
                if (null != mHomeModel.getPalletCheckBillList() && mHomeModel.getPalletCheckBillList().size() >0){
                    orderId = mHomeModel.getPalletCheckBillList().get(0).getPalletCheckId();
                    intent = new Intent(this,NewTpCheckDetailsActivity.class);
                    intent.putExtra(ExtraConfig.TypeCode.FROM_INTTENT,ExtraConfig.IntentType.FROM_INTENT_BY_SHOU);
                    intent.putExtra(ExtraConfig.TypeCode.INTENT_ORDER_ID,orderId);
                    startActivity(intent);
                }else {
