...
import com.etuo.kucun.ui.adapter.NewGoodsBuyInDetailsAdapter;
...
import static com.etuo.kucun.utils.UrlTools.GOODS_BATCH_ACCEPTANCE;
import static com.etuo.kucun.utils.UrlTools.GOODS_BUY_IN_GETBILL;
import static com.etuo.kucun.utils.UrlTools.GOODS_SINGLE_OPERATION;
 ...
 * @version :
 * @date :2020/02/08
 * @ProjectNameDescribe :采购入库 等详情操作界面
 * 修订历史：
 * ================================================
 */

public class NewGoodsBuyInDetailsActivity extends BaseActivity implements NewGoodsBuyInDetailsAdapter.OnClickBtItem {
    ...
    private List<BuyInDetailsModel.GatherListBean> mOutBoundListModel = new ArrayList<>();
    private NewGoodsBuyInDetailsAdapter mAdapter;
    private BuyInDetailsModel mOutBoundModel;
    String orderId;
    ...
    private List<BuyInDetailsModel.GatherListBean.DetailListBean> detailListBeanList = new ArrayList<>();//批量操作是缓存的list ,只有在按下确定键开始添加,松开停止 然后提交数据
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new NewGoodsBuyInDetailsAdapter(this, fromType,getContentView(this));
...
//            }
        ...
        OkGo.get(UrlTools.getInterfaceUrl(GOODS_BUY_IN_GETBILL))
                .tag(this)
                .params("token", PreferenceCache.getToken())
                .params("goodsStorageInId", orderId)


                .execute(new DialogCallback<LzyResponse<BuyInDetailsModel>>(this, "加载中..") {
                    @Override
                    public void onSuccess(LzyResponse<BuyInDetailsModel> responseData, Call call, Response response) {
...
//                            lvOutList.onRefreshComplete();
//                        }
                        ShowToast.tCustom(NewGoodsBuyInDetailsActivity.this, e.getMessage());
                    }
                });
     ...
     * @return
     */
    private List<BuyInDetailsModel.GatherListBean> getNewRankList(List<BuyInDetailsModel.GatherListBean> listBeen) {

        List<BuyInDetailsModel.GatherListBean> listBeanList = new ArrayList<>();
        for (BuyInDetailsModel.GatherListBean listBean : listBeen) {
            List<BuyInDetailsModel.GatherListBean.DetailListBean> list = listBean.getDetailList();

            Collections.sort(list, new Comparator<BuyInDetailsModel.GatherListBean.DetailListBean>() {
                @Override
                public int compare(BuyInDetailsModel.GatherListBean.DetailListBean o1, BuyInDetailsModel.GatherListBean.DetailListBean o2) {
     ...
     * @param result
     */
    private void setRefreshList(boolean isPullDown, List<BuyInDetailsModel.GatherListBean> result) {
...
//            lvOutList.getRefreshableView().setSelectedChild(ints[0],ints[1],false);
    ...
    private int[] getUpPosition(String palletNo) {
        ...
        for (int i = 0; i < mOutBoundListModel.size(); i++) {
            BuyInDetailsModel.GatherListBean gatherListBean = mOutBoundListModel.get(i);
            List<BuyInDetailsModel.GatherListBean.DetailListBean> list = mOutBoundListModel.get(i).getDetailList();
    ...
    public void upLoadOrderComfrim() {
        LogUtil.d("token : " + PreferenceCache.getToken());
        OkGo.post(UrlTools.getInterfaceUrl(GOODS_SINGLE_OPERATION))
                .tag(this)
                .params("token", PreferenceCache.getToken())
                .params("goodsStorageInId", NullToStr(orderId))
                .params("status", "2") //状态（0：待入库，1：已入库，2：不通过）
                .execute(new DialogCallback<LzyResponse<String>>(this, "加载中..") {
                    ...
                    public void onError(Call call, Response response, Exception e) {
                        super.onError(call, response, e);
                        ShowToast.tCustom(NewGoodsBuyInDetailsActivity.this, NullToStr(e.getMessage()));
                    }
                });
    }
    ...
    public void commitListBing(String bindingParamJson, final String type, final String palletNum) {
        LogUtil.d("token : " + PreferenceCache.getToken());
        OkGo.post(UrlTools.getInterfaceUrl(GOODS_BATCH_ACCEPTANCE))
                .tag(this)
                .params("token", PreferenceCache.getToken())
                ...
                .execute(new DialogCallback<LzyResponse<String>>(this, "加载中..") {
                    ...
                    public void onError(Call call, Response response, Exception e) {
                        ...
                        ShowToast.tCustom(NewGoodsBuyInDetailsActivity.this, e.getMessage().toString());
                    }
                });
    }
     ...
     */
    @Override
    public void myOrderOkClick(int position, final BuyInDetailsModel.GatherListBean.DetailListBean singleData) {
...
//                            List<BuyInDetailsModel.GatherListBean.DetailListBean> listBeen = new ArrayList<>();
...
//            p.show(ExtraConfig.TypeCode.DIALOG_CAOZUO_TO_EXIT);
     ...
     */
    @Override
    public void myOrderCancelClick(int position, final BuyInDetailsModel.GatherListBean.DetailListBean singleData) {
...
//            p.show(ExtraConfig.TypeCode.DIALOG_CANCEL_TO_EXIT);
     ...
     * @return list->json
     */
    private String getCommitJosn(List<BuyInDetailsModel.GatherListBean.DetailListBean> singleData, String status) {
        ...
        for (int i = 0; i < singleData.size(); i++) {
            ...
            commitModel.setGoodsStorageInDetailId(singleData.get(i).getGoodsStorageInDetailId());
     ...
     * @return 扫描托盘是不是可以提交
     */
    private BuyInDetailsModel.GatherListBean.DetailListBean getCheckBean(String palletId, String status) {

        for (BuyInDetailsModel.GatherListBean listBean : mOutBoundListModel) {
            for (BuyInDetailsModel.GatherListBean.DetailListBean detailListBean : listBean.getDetailList()) {
    ...
    private boolean isInList(String palletId) {
        ...
        {
            for (BuyInDetailsModel.GatherListBean.DetailListBean listBean : detailListBeanList) {
    ...
    private void commitTpByScanId(String code, String status) {
        BuyInDetailsModel.GatherListBean.DetailListBean singleData = getCheckBean(code, status);
...
//    private String wrData = "A000012020022112345678202002211234001001";
    ...
    private void initRfid() {
        ...
        mRfidOperationUtils.readTagCallBack(new RfidOperationUtils.readTagCallBack() {
            ...
            public void MoreStepReadTagCallBack(String strTid, String strEpc) {
                ...
                BuyInDetailsModel.GatherListBean.DetailListBean singleData = getCheckBean(strEpc, "1");
    ...
    public class WriteDataTask extends AsyncTask<String, Integer, Boolean> {
        ...
        List<BuyInDetailsModel.GatherListBean.DetailListBean> newDetailListBeen;
